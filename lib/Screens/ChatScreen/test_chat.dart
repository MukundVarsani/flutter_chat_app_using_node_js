...
import 'package:chat_app_with_backend/main.dart';
...
class _ChatScreeState extends State<ChatScree> {
  ...
  Stream<List<MessageModel>> _getChatStream() async* {
    try {
      List<MessageModel>? msgs = await BlocProvider.of<GetMessageCubit>(context)
          .getMessages(recipientId: widget.recipientId);

      Vx.log(msgs?.last.text.toString());
      if (msgs != null) {
        yield msgs;
      }
    } catch (e) {
      ...
      body: Hero(
        ...
        child: Stack(
          ...
          children: [
                    ...
                    return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return MessageBubble(
                              isSender: isSender,
                              message: snapshot.data?[index].text ?? "null");
                        });
            ...
            IconButton(
              onPressed: () {
                // if (_messageController.text.trim().isNotEmptyAndNotNull) {
                BlocProvider.of<GetMessageCubit>(context).sendMessage(
                    text: "Stream", recipientId: widget.recipientId);
                // sendMessage(
                //  "Stream"
                // );
                _messageController.clear();
